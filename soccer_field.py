...
class Field:
    ...
    def G(self, x, u):
        ...
        sum_angle = minimized_angle(prev_theta+rot1)
        return np.array([
            [1, 0, -trans*np.sin(sum_angle)],
            [0, 1, trans*np.cos(sum_angle)],
            [0, 0, 1]
        ])
    ...
    def V(self, x, u):
        ...
        prev_x, prev_y, prev_theta = x.ravel()
        rot1, trans, rot2 = u.ravel()
        angle_sum = minimized_angle(prev_theta+rot1)
        # MY IMPLEMENTATION HERE
        return np.array([
            [-trans*np.sin(angle_sum), np.sin(angle_sum), 0],
            [trans*np.cos(angle_sum), np.sin(angle_sum), 0],
            [1, 0, 1]
        ])
